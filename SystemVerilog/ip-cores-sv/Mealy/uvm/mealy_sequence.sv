...
//------------------------------------------------------------------------------
class mealy_sequence extends uvm_sequence #(pkt);
    `uvm_object_utils(mealy_sequence)
    ...
    // Creates the sample event; get_global can be used to retrieve an event or to create one if it doesn't exists yet.
    function new (string name = "mealy_sequence");
        super.new(name);
...
virtual task body();
    ...
    while (current_coverage < 100.0) begin
        ...
        // Gets the current coverage; Casts the result into void type.
